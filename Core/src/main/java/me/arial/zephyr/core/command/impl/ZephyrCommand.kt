...
//            })
        ...
        command
            .withSubcommands(
                CommandAPICommand("modules")
                    .withOptionalArguments(GreedyStringArgument("any"))
                    .executes(CommandExecutor { sender, args ->
                        if (args.args.isNotEmpty()) {
                            sender.sendColoredMessage("$prefix &fИспользование: &c/zephyr modules")
                            return@CommandExecutor
                        }
                        modulesHandler(sender)
                    }),
                CommandAPICommand("module")
                    .withOptionalArguments(
                        ...
                        GreedyStringArgument("any")

                    )
                    .executes(dev.jorel.commandapi.executors.CommandExecutor { sender, args ->
                        if(args.args.size != 2) {
                            sender.sendColoredMessage("$prefix Использование: &c/zephyr module <модуль> <info/unload/reload>")
                            return@CommandExecutor
                        }
                        ...
                        when(args.args[1]) {
                            ...
                            else -> sender.sendColoredMessage("$prefix Использование: &c/zephyr module <модуль> <info/unload/reload>")
                        }
                    }),
                CommandAPICommand("load")
                    .withOptionalArguments(
                        TextArgument("file")
                            .includeSuggestions { suggestionInfo, suggestionsBuilder ->
                                val files = Zephyr.instance.zephyrModuleManager.moduleFolder.listFiles {
                    ...
                    .executes(dev.jorel.commandapi.executors.CommandExecutor { sender, args ->
                        if (args.args.size != 1) {
                            sender.sendColoredMessage("$prefix Использование: &c/zephyr load <модуль>")
                            return@CommandExecutor
                        }
